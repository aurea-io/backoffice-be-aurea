...
export class RestaurantService {
  ...
  async updateOrder(tenantId: string, id: string, dto: UpdateOrderDto) { const order = await this.prisma.order.findFirst({ where: { id, tenantId } }); if (!order) throw new NotFoundException('Pedido no encontrado.'); const { deliveryEta, courierName, courierPhone, ...state } = dto; return this.prisma.order.update({ where: { id }, data: { ...state, courierName: courierName?.trim(), courierPhone: courierPhone?.trim(), deliveryEta: deliveryEta ? new Date(deliveryEta) : undefined }, include: { lines: true } }); }
