...
@FeatureDomain(FeatureConstants.INVENTORY)
export class InventoryController {
  constructor(private readonly inventory: InventoryService) {}
  @Get() @RequireRead() list(@CurrentTenant() tenant: TenantContext) { return this.inventory.list(tenant.tenantId); }
  @Post() @Roles(Role.OWNER, Role.MANAGER) @RequireWrite() create(@CurrentTenant() tenant: TenantContext, @Body() dto: CreateInventoryDto) { return this.inventory.create(tenant.tenantId, dto); }
  @Post(':id/adjust') @Roles(Role.OWNER, Role.MANAGER) @RequireWrite() adjust(@CurrentTenant() tenant: TenantContext, @Param('id') id: string, @Body() dto: AdjustInventoryDto) { return this.inventory.adjust(tenant.tenantId, id, dto); }
}
