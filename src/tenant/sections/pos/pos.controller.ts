...
export class PosController {
  ...
  @Get('cash') @RequireRead() current(@CurrentTenant() tenant: TenantContext) { return this.pos.current(tenant.tenantId); }
  @Post('cash/open') @Roles(Role.OWNER, Role.MANAGER, Role.CASHIER) @RequireWrite() open(@CurrentTenant() tenant: TenantContext, @Body() dto: OpenCashDto) { return this.pos.open(tenant.tenantId, dto); }
  @Post('cash/close') @Roles(Role.OWNER, Role.MANAGER, Role.CASHIER) @RequireWrite() close(@CurrentTenant() tenant: TenantContext, @Body() dto: CloseCashDto) { return this.pos.close(tenant.tenantId, dto); }
}
