...
import type { CreateOrderDto, UpdateOrderDto } from './orders.dto.js';
import { CouponsService } from '../../marketing/coupons/coupons.service.js';
...
export class OrdersService {
  ...
  async createOrder(tenantId: string, dto: CreateOrderDto) {
    ...
    if (dto.tableId && !(await this.prisma.restaurantTable.findFirst({ where: { id: dto.tableId, tenantId } }))) {
      throw new BadRequestException('La mesa no pertenece al comercio.');
    }
    const catalog = await this.prisma.catalogItem.findMany({
      where: { tenantId, id: { in: dto.lines.map((line) => line.catalogItemId) }, isActive: true },
    });
  ...
  async getOrderTicket(tenantId: string, id: string) {
    const order = await this.prisma.order.findFirst({
      where: { id, tenantId },
      include: { lines: { include: { catalogItem: { select: { title: true } } } } },
    });
    if (!order) throw new NotFoundException('Pedido no encontrado.');
    const totalCents = order.lines.reduce((sum, line) => sum + line.quantity * line.unitPriceCents, 0) - order.discountCents;
    const parts = new Map<string, { totalCents: number; lines: Array<{ title: string; quantity: number; amountCents: number }> }>();
    order.lines.forEach((line) => {
      ...
      current.lines.push({ title: line.catalogItem.title, quantity: line.quantity, amountCents });
    ...
    return {
      ...
      parts: [...parts.entries()].map(([person, value]) => ({ person, ...value })),
    };
  }
  ...
  async issueFiscalReceipt(tenantId: string, id: string) {
    const order = await this.prisma.order.findFirst({
      where: { id, tenantId },
      include: { lines: { include: { catalogItem: { select: { title: true } } } }, fiscalReceipt: true },
    });
    if (!order) throw new NotFoundException('Pedido no encontrado.');
    if (order.fiscalReceipt) return order.fiscalReceipt;
    const totalCents = order.lines.reduce((sum, line) => sum + line.quantity * line.unitPriceCents, 0) - order.discountCents;
