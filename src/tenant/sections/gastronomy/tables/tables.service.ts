...
import type { CreateTableDto, UpdateTableDto, CreateTableBookingDto, UpdateTableBookingDto } from './tables.dto.js';
...
export class TablesService {
  ...
  listTables(tenantId: string) {
    return this.prisma.restaurantTable.findMany({
      where: { tenantId },
      include: { orders: { where: { status: { not: 'paid' as any } }, include: { lines: true } } },
      orderBy: { number: 'asc' },
    });
  }
  ...
  createTable(tenantId: string, dto: CreateTableDto) {
    return this.prisma.restaurantTable.create({
      data: { tenantId, number: dto.number, seats: dto.seats ?? 2 },
    });
  }
  ...
  listTableBookings(tenantId: string, from?: string, to?: string) {
    return this.prisma.tableBooking.findMany({
      where: { tenantId, ...(from || to ? { date: { gte: from ? tableDay(from) : undefined, lte: to ? tableDay(to) : undefined } } : {}) },
      include: { table: true },
  ...
  async createTableBooking(tenantId: string, dto: CreateTableBookingDto) {
    ...
    const candidates = dto.tableId
      ? await this.prisma.restaurantTable.findMany({ where: { id: dto.tableId, tenantId, seats: { gte: dto.partySize } } })
      : await this.prisma.restaurantTable.findMany({ where: { tenantId, seats: { gte: dto.partySize }, status: { not: 'billing' as any } }, orderBy: { number: 'asc' } });
    ...
    const existing = await this.prisma.tableBooking.findMany({
      where: { tenantId, date: tableDay(dto.date), status: { not: 'canceled' as any }, tableId: { in: candidates.map((table) => table.id) } },
  ...
  async updateTableBooking(tenantId: string, id: string, dto: UpdateTableBookingDto) {
    const booking = await this.prisma.tableBooking.findFirst({ where: { id, tenantId } });
    if (!booking) throw new NotFoundException('Reserva de mesa no encontrada.');
    return this.prisma.tableBooking.update({ where: { id }, data: dto, include: { table: true } });
  }
  ...
  async tableBookingAvailability(publicId: string, date: string, partySize: number) {
    ...
    const slots = Array.from({ length: 19 }, (_, index) => 12 * 60 + index * 30)
      .filter((start) => start + 120 <= 22 * 60 && tables.some((table) => !bookings.some((booking) => booking.tableId === table.id && start < tableMinutes(booking.startTime) + booking.durationMin && start + 120 > tableMinutes(booking.startTime))))
